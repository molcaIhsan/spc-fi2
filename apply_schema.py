...
def main():
    ...
    # schema.sql hardcodes "spc" for CREATE SCHEMA/SET search_path -- fine as
    # the readable default, but if SPC_PG_SCHEMA points somewhere else, the
    # actual DDL has to follow it, not silently keep creating tables in 'spc'
    # regardless. Only the two functional lines are touched, not the file's
    # comments, so schema.sql itself stays readable as documentation.
    if pg.schema != "spc":
        sql_text = sql_text.replace(
            "CREATE SCHEMA IF NOT EXISTS spc;", f"CREATE SCHEMA IF NOT EXISTS {pg.schema};"
        ).replace(
            "SET search_path TO spc;", f"SET search_path TO {pg.schema};"
        )
        print(f"Applying to schema '{pg.schema}' (SPC_PG_SCHEMA override, not the default 'spc')")
